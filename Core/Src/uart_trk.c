...
void TrkUart_Init(void)
{
    ...
    // Предполагается, что MX_USART3_UART_Init() уже вызывался в main()
    // Запустить приём по прерыванию
    TrkUart_StartRxIT();
}
...
/**
 * @brief HAL callback приёма по IT для любого UART.
 *        Мы обрабатываем только USART3.
 */
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART3) {
        // Вычислить следующий индекс для головы
        uint16_t next = (trk_rx_head + 1) % TRK_RX_BUF_SIZE;
        // Если не переполнение, обновить голову
        if (next != trk_rx_tail) {
            trk_rx_head = next;
        }
        // И перезапустить приём
        TrkUart_StartRxIT();
    }
}
